...
def format_dataset_for_mc(dataset, dataset_name) -> Tuple[List, List, List]:
    ...
    for datapoint in dataset:
        question = " ".join(
            [datapoint[col] for col in dataset_columns[dataset_name]["question"]]
        )
...
def process_book_file(
    book_path: str, tokenizer: GPT2Tokenizer, max_length: Optional[int] = None
) -> List[str]:
    lines = []
    ...
    def __getitem__(self, idx):
        ...
        return {
            "input_ids": torch.stack(
                [torch.tensor(inp["input_ids"]) for inp in inputs]
            ),
            "attention_mask": torch.stack(
                [torch.tensor(inp["attention_mask"]) for inp in inputs]
            ),
...
def validate_mcq(
    model, valid_dataloader_mcq, using_deepspeed=True
) -> Tuple[float, int]:
    model.eval()
    ...
    with torch.no_grad():
        for batch in tqdm(valid_dataloader_mcq):
            ...
            logits = torch.gather(logits, dim=3, index=input_ids.unsqueeze(3)).squeeze(
                3
            )  # (1, num_options, seq_len)
...
def validate_lm(
    model, valid_dataloader_lm, using_deepspeed=True, local_rank=None
) -> float:
    model.eval()
    with torch.no_grad():
...
def train_all_options(
    ...
    local_rank=-1,
    n_epochs=10,
    convert_to_fp32_now=False,
    log_steps_frequency=1000,
    fold=0,
):
    if len(sel_datasets) == 1:
        train_qs, train_options, train_answers = format_dataset_for_mc(
            train_set[0], sel_datasets[0]
        )
        valid_qs, valid_options, valid_answers = format_dataset_for_mc(
            valid_set[0], sel_datasets[0]
        )
    else:
    ...
    train_set = MCDataset(
        train_qs, train_options, train_answers, tokenizer, is_one_indexed=is_one_indexed
    )
    valid_set = MCDataset(
        valid_qs, valid_options, valid_answers, tokenizer, is_one_indexed=is_one_indexed
    )

    lm_train_set = LMDataset(
        lm_train_set["text"], tokenizer, tokenizer.model_max_length
    )
    lm_valid_set = LMDataset(
        lm_valid_set["text"], tokenizer, tokenizer.model_max_length
    )
    ...
    combined_train_dataloader = DataLoader(
        combined_dataset_train, batch_size=1, shuffle=True
    )
    ...
    if using_deepspeed:
        ...
        model, optimizer, _, _ = deepspeed.initialize(
            model=model, config_params=dsconfig
        )
    else:
    ...
    for epoch in range(num_epochs):
        ...
        for i, batch in enumerate(tqdm(combined_train_dataloader)):
            ...
            if fp16:
                with autocast():
                    ...
                    if task_type == "mcq":
                        logits = outputs.logits  # (1, num_options, seq_len, vocab_size)
                        logits = torch.gather(
                            logits, dim=3, index=input_ids.unsqueeze(3)
                        ).squeeze(
                            3
                        )  # (1, num_options, seq_len)
                        logits = logits * attention_mask
            ...
            if (i % log_steps_frequency == 0) and (i != 0):
                if lm_task_p != 1:
                    logging.info(
                        f"Avg train loss (MCQ): {running_loss_mcq / num_mcq_questions}"
                    )
                if lm_task_p != 0:
                    logging.info(
                        f"Avg train loss (LM): {running_loss_lm / num_lm_questions}"
                    )
                ...
                if lm_task_p != 1:
                    results_train.update(
                        {
                            "mcq_loss": running_loss_mcq / log_steps_frequency,
                            "mcq_accuracy": num_correct / num_mcq_questions,
                        }
                    )
                elif lm_task_p != 0:
                    results_train.update(
                        {"lm_loss": running_loss_lm / log_steps_frequency}
                    )
                ...
                if not using_deepspeed or (using_deepspeed and local_rank == 0):
                    total_loss_valid_mcq, num_correct_val = validate_mcq(
                        model, valid_dataloader_mcq, using_deepspeed=using_deepspeed
                    )

                    logging.info(
                        f"Avg valid loss (MCQ): {total_loss_valid_mcq / len(valid_dataloader_mcq)}"
                    )
                    if using_wandb and (
                        not using_deepspeed or (using_deepspeed and local_rank == 0)
                    ):
                        if lm_task_p != 1:
                            wandb.log(
                                {
                                    ...
                                    "valid_mcq_loss": total_loss_valid_mcq
                                    / len(valid_dataloader_mcq),
                                    "valid_mcq_accuracy": num_correct_val
                                    / len(valid_dataloader_mcq),
                                }
                            )
                        else:
                ...
                if not using_deepspeed or (using_deepspeed and local_rank == 0):
                    total_loss_lm = validate_lm(
                        model, valid_dataloader_lm, using_deepspeed=using_deepspeed
                    )
                    logging.info(
                        f"Avg valid loss (LM): {total_loss_lm / len(valid_dataloader_lm)}"
                    )
                    if using_wandb:
                        if lm_task_p != 0:
                            wandb.log(
                                {
                                    ...
                                    "valid_lm_loss": total_loss_lm
                                    / len(valid_dataloader_lm),
                                }
                            )
                        else:
                            wandb.log(
        ...
        if not using_deepspeed or (using_deepspeed and local_rank == 0):
            ...
            logging.info(
                f"Avg valid loss: {total_loss_valid_mcq / len(valid_dataloader_mcq)}"
            )
            if using_wandb:
                if lm_task_p != 1:
                    wandb.log(
                        {
                            "epoch": epoch,
                            "train_mcq_loss": total_epoch_loss_mcq
                            / total_mcq_questions,
                            "train_mcq_accuracy": total_epoch_num_correct
                            / total_mcq_questions,
                            "valid_mcq_loss": total_loss_valid_mcq
                            / len(valid_dataloader_mcq),
                            "valid_mcq_accuracy": num_correct_val
        ...
        logging.info("Evaluating the LM task")
        if not using_deepspeed or (using_deepspeed and local_rank == 0):
            total_loss_valid_lm = validate_lm(
                model, valid_dataloader_lm, using_deepspeed=using_deepspeed
            )

            logging.info(
                f"Avg valid loss: {total_loss_valid_lm / len(valid_dataloader_lm)}"
            )
                    ...
                    wandb.log(
                        {
                            ...
                            "valid_lm_loss": total_loss_valid_lm
                            / len(valid_dataloader_lm),
                        }
                    )
                else:
        ...
        if (total_epoch_loss / denom) < best_val_loss:
            logging.info(
                f"EPOCH {epoch}: New best model with loss {total_epoch_loss / denom}"
            )
            ...
            logging.info(
                f"No improvement, the best model still has loss {best_val_loss}"
            )
            if early_stopping > 0:
                patience -= 1
                if patience == 0:
    ...
    if not using_deepspeed or (using_deepspeed and args.local_rank == 0):
        ...
        total_loss_test_lm = validate_lm(
            model,
            test_dataloader_lm,
            using_deepspeed=using_deepspeed,
            local_rank=f"cuda:{args.local_rank}",
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Finetune a model on commonsense datasets of different types"
    )
    parser.add_argument(
        "--model_name",
        type=str,
        default="gpt2",
        help="Model name, or path to a saved model",
    )
    parser.add_argument(
        "--dataset_type",
        type=str,
        default="metaphors",
        choices=["metaphors", "emotions", "physical"],
        help="Type of dataset",
    )
    parser.add_argument(
        "--specific_dataset",
        type=str,
        help="Specific dataset to use (will not train on all datasets of a type)",
    )
    parser.add_argument("--output_dir", type=str, help="Output directory")
    parser.add_argument(
        "--training_style",
        default="all_options",
        choices=["correct_options", "all_options"],
        help="Training style to adapt MC to LMHead model",
    )
    parser.add_argument(
        "--early_stopping",
        type=int,
        default=0,
        help="Use early stopping with patience of this many epochs",
    )
    parser.add_argument(
        "--num_epochs",
        type=int,
        default=10,
        help="Number of epochs to train for. Mutually exclusive with early stopping.",
    )
    parser.add_argument("--wandb", action="store_true", help="Use wandb for logging")
    parser.add_argument(
        "--debug", action="store_true", help="Train on a small number of examples"
    )
    ...
    parser.add_argument(
        "--is_local_model",
        action="store_true",
        help="Start training from a locally saved model. You need to specify this for training to work.",
    )
    parser.add_argument(
        "--lm_percentage",
        default=0,
        type=float,
        help="Will multitask between commonsense reasoning (MCQ) training and LM training, where p represents LM percentage of batches. Please check lr in the deepspeed config if doing continued lm training on an already trained model.",
    )
    parser.add_argument(
        "--book4_cv",
        action="store_true",
        help="Run cross validation for perplexity on the 4th book of harry potter.",
    )
    parser.add_argument(
        "--log_steps_freq",
        type=int,
        help="Number of steps after which to log (plots will be made for both epochs and steps)",
        default=1000,
    )
    parser.add_argument(
        "--wandb_offline", action="store_true", help="Run wandb offline"
    )
    ...
    if args.wandb and (args.local_rank == -1 or args.local_rank == 0):
        ...
        if args.specific_dataset is not None:
            if args.wandb_offline:
                run = wandb.init(
                    mode="offline",
                    project="commonsense_lm_training",
                    config=args,
                    name=f"{args.model_name}_{args.specific_dataset}_{args.training_style}",
                )
            else:
                run = wandb.init(
                    project="commonsense_lm_training",
            ...
            if args.wandb_offline:
                run = wandb.init(
                    ...
                    project="commonsense_lm_training",
                    config=args,
                    name=f"{args.model_name}_{args.dataset_type}_{args.training_style}",
                )
            else:
                run = wandb.init(
                    project="commonsense_lm_training",
    ...
    if args.lm_percentage == 1:
        logging.info(
            "lm_percentage of 1 passed, this run will just run language modelling training."
        )
    elif args.lm_percentage == 0:
        logging.info(
            "lm_percentage of 0 passed, this will just finetune on the commonsense task. The model will not be usable as an LM afterwards."
        )

    hf_datasets_train = [
        datasets.load_dataset(dataset_name, split="train")
        for dataset_name in sel_datasets
    ]
    hf_dataset_valid = [
        datasets.load_dataset(dataset_name, split="validation")
        for dataset_name in sel_datasets
    ]
    ...
    if args.book4_cv:
        ...
        for fold, (train_idx, test_idx) in enumerate(kfold.split(book4)):
            ...
            if args.debug:
                book4_train, book4_test, book4_valid = (
                    book4_train[:100],
                    book4_test[:100],
                    book4_valid[:100],
                )
            ...
            trained_model = train_all_options(
                model,
                tokenizer,
                hf_datasets_train,
                hf_dataset_valid,
                ...
                log_steps_frequency=args.log_steps_freq,
                fold=fold,
            )

    else:
        if args.training_style == "correct_options":
            model = train_correct_options_only(
                model,
                tokenizer,
                hf_datasets_train,
                hf_dataset_valid,
                early_stopping=args.early_stopping,
                lm_task_p=args.lm_percentage,
                fp16=args.fp16,
                dsconfig=dschf,
                using_deepspeed=using_deepspeed,
                using_wandb=args.wandb,
